...
function ProductsContent() {
  ...
  // ✅ تشغيل مزامنة الـ Offline Queue في صفحة المنتجات أيضاً
  // حتى لو المستخدم لم يفتح صفحة المبيعات الرئيسية
  useOfflineSync();
  ...
  useEffect(() => {
    const checkLock = async () => {
      ...
      // ✅ Offline: السماح بالوصول بدون التحقق من Firestore
      ...
      // Online: التحقق من الصلاحيات من Firestore
      try {
        const q = query(
          collection(db, "users"),
  ...
  // ✅ استخدام useProducts hook - يعرض البيانات من localStorage عند offline ومن Firestore عند online
  // لا حاجة لـ useEffect منفصل - useProducts يتولى ذلك
  ...
  const getNextCode = useCallback(async () => {
    ...
    // ✅ دائماً نقرأ من localStorage أولاً (أسرع وأكثر موثوقية)
    try {
    ...
    // ✅ Online: محاولة قراءة من Firestore كنسخة احتياطية (فقط لو Online)
    if (typeof window !== "undefined" && navigator.onLine) {
      try {
        const q = query(
        ...
        // نكمل بدون إظهار خطأ للمستخدم
      }
    }

    // ✅ القيمة الافتراضية إذا لم نجد أي كود
    return 1000;
  }, []);
  ...
  const handleAddProduct = useCallback(async () => {
    ...
    try {
      ...
      // ✅ استخدام offlineAdd لضمان العمل في Online و Offline
      try {
        const result1 = await offlineAdd("lacosteProducts", productObj);
        
        // التحقق من نجاح العملية الأولى
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // حذف المنتج من lacosteProducts (مع دعم offline)
      const deleteResult = await offlineDelete("lacosteProducts", product.id);

      // ✅ لا حاجة لتحديث الـ state - useProducts hook سيتولى ذلك من خلال event listener
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      // ✅ استخدام offlineUpdate (يحفظ في localStorage و Firestore عند online)
      const updateData = {
        name: form.name || "",
      ...
      if (result.offline) {
        success("تم تحديث المنتج (سيتم المزامنة عند الاتصال بالإنترنت)");
      } else {
              ...
              <div className={styles.headerControls}>
                ...
                <div className={styles.filterDropdown}>
                  ...
                  >
                    ...
                    <option value="كوتشي">كوتشي</option>
                  </select>
                </div>
